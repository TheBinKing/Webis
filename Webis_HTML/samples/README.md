...
# Monitor task status
while True:
    status = requests.get(f"http://localhost:9000/tasks/{async_task_id}").json()['status']
    if status == 'completed':
        break
    time.sleep(5)
...
- Model server (port 9065) [started]
- Web API server (port 9000) [running] 
Example startup command: python -m uvicorn app:app --host 0.0.0.0 --port 9000
